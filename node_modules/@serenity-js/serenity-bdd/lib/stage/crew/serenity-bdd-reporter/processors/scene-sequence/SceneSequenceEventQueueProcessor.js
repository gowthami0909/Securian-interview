...
class SceneSequenceEventQueueProcessor extends EventQueueProcessor_1.EventQueueProcessor {
    ...
    onActivityFinished(context) {
        return (event) => context
            .with((0, transformations_1.activityFinished)(event.activityId, event.details.name, event.outcome, event.timestamp));
    }
    onSceneFinished(context) {
        return (event) => context
            .with((0, transformations_1.activityFinished)(event.sceneId, event.details.name, event.outcome, event.timestamp))
